...
void ShudderCoreTests::appIdentityIsShudder()
{
  ...
  QCOMPARE(QStringLiteral(SHUDDER_VERSION), QStringLiteral("0.1.4"));
}
...
void ShudderCoreTests::releaseMetadataIsConsistent()
{
  ...
  QVERIFY(changelogText.contains("## [0.1.4] - 2026-07-29"));
  ...
  QVERIFY(metadataText.contains("<release version=\"0.1.4\" date=\"2026-07-29\">"));
  QVERIFY(metadataText.contains("<release version=\"0.1.3\" date=\"2026-07-25\">"));
  ...
  QFile appImageBuild(QStringLiteral(SHUDDER_SOURCE_DIR "/packaging/appimage/build-appimage.sh"));
  QVERIFY(appImageBuild.open(QIODevice::ReadOnly));
  const QByteArray appImageBuildText = appImageBuild.readAll();
  QVERIFY(appImageBuildText.contains("libleancrypto.so.1"));
  QVERIFY(appImageBuildText.contains("install -m755 \"$leancrypto_source\""));

  QFile appImageVerification(QStringLiteral(SHUDDER_SOURCE_DIR "/packaging/appimage/verify-appimage.sh"));
  QVERIFY(appImageVerification.open(QIODevice::ReadOnly));
  const QByteArray appImageVerificationText = appImageVerification.readAll();
  QVERIFY(appImageVerificationText.contains("readelf --use-dynamic --histogram"));
  QVERIFY(appImageVerificationText.contains("timeout --kill-after=2s 10s \"$root/AppRun\""));
}
